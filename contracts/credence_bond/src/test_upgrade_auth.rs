...
use crate::{
    upgrade_auth::{self, UpgradeRole, UpgradeStatus, UpgradeAuthorization, UpgradeProposal, UpgradeRecord},
    CredenceBondClient,
};
...
fn test_upgrade_authorization_initialization() {
    ...
    assert_eq!(upgrade_auth::get_upgrade_role(&env, &admin), UpgradeRole::Upgrader);
...
fn test_grant_and_revoke_upgrade_authorization() {
    ...
    assert_eq!(upgrade_auth::get_upgrade_role(&env, &user2), UpgradeRole::Proposer);
...
fn test_upgrade_proposal_and_approval() {
    ...
    let proposal_id = upgrade_auth::propose_upgrade(
        &env,
        &proposer,
        new_impl,
        Vec::new(&env),
